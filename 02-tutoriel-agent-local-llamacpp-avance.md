# 02 — Tutoriel agent local avancé : OpenClaude + llama.cpp compilé à la main
...
> **À lire avant** : si tu veux juste que ça marche, prends le guide [`01-installation-openclaude-multiplatform.md`](./01-installation-openclaude-multiplatform.md). Ce document-ci est pour les power users.
...
**Inconvénient principal** : le build prend 10-30 min et il y a des pièges (surtout sous Windows). Si tu n'as pas un besoin très spécifique, **utilise Ollama** (cf. [doc 01](./01-installation-openclaude-multiplatform.md)).
...
Documenté dans [`docs/04-guide-complet-troubleshooting.md`](../04-guide-complet-troubleshooting.md).
...
Pour les détails (permissions Mac/Linux, prefix npm), voir [`01-installation-openclaude-multiplatform.md` section 4.5](./01-installation-openclaude-multiplatform.md#45-installer-openclaude).
...
→ Config `~/.claude.json` qui prend le dessus sur tes variables d'env. Voir [`docs/06-troubleshooting-config-globale-openclaude.md`](../06-troubleshooting-config-globale-openclaude.md).
...
### Arborescence type du projet

```
gemma-agressif-2/
├── docs/
│   ├── 00-installation-windows-ollama-gguf.md
│   ├── 01-tutoriel-agent-local.md            (tutoriel original Windows)
│   └── openclaude/
│       ├── 01-installation-openclaude-multiplatform.md
│       └── 02-tutoriel-agent-local-llamacpp-avance.md   (ce document)
├── scripts/
│   ├── 00-clean-build.ps1
│   ├── 01-check-prereqs.ps1
│   ├── 02-clone-llama.ps1
│   ├── 03-build-llama.ps1                    ← compilation llama.cpp
│   ├── 04-setup-python-hf.ps1
│   ├── 05-download-model.ps1
│   ├── 06-install-openclaude.ps1
│   ├── 07-start-agent.ps1                    ← lance llama-server + openclaude
│   ├── 99-stop-agent.ps1
│   └── README.md
├── llama.cpp/                                (cloné, ignoré par git)
├── .venv/                                    (créé par 04, ignoré par git)
└── README.md
```
...
```bash
# macOS / Linux — équivalent shell à écrire
./run-agent.sh    # ou directement les commandes de la section 6
```
...
- [`01-installation-openclaude-multiplatform.md`](./01-installation-openclaude-multiplatform.md) — la version simple (Ollama / LM Studio) pour démarrer rapidement
- [`docs/00-installation-windows-ollama-gguf.md`](../00-installation-windows-ollama-gguf.md) — utiliser un GGUF custom Hugging Face avec Ollama
- [`docs/04-guide-complet-troubleshooting.md`](../04-guide-complet-troubleshooting.md) — les 10+ erreurs Windows documentées
- [`docs/05-guide-utilisation.md`](../05-guide-utilisation.md) — utiliser OpenClaude au quotidien
- [`docs/06-troubleshooting-config-globale-openclaude.md`](../06-troubleshooting-config-globale-openclaude.md) — quand `~/.claude.json` ignore tes variables
